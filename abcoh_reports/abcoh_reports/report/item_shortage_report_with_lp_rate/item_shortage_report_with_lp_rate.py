...
def get_data(filters):
	...
	query = (
		...
		.select(
			...
			item.item_name,
			item.description,
			item.last_purchase_rate,         # 🔹 new field
			item.custom_last_supplier        # 🔹 new field
		)
		.where(
			(item.disabled == 0)
			& (bin.projected_qty < 0)
...
def get_columns():
	columns = [
		...
		{"label": _("Description"), "fieldname": "description", "fieldtype": "Data", "width": 120},
  		{"label": _("Last Purchase Rate"), "fieldname": "last_purchase_rate", "fieldtype": "Currency", "width": 120},
		{"label": _("Last Supplier"), "fieldname": "custom_last_supplier", "fieldtype": "Link", "options": "Supplier", "width": 180},
	]
